...
def gram_matrix(A):
    ...
    It is obtained by multplying the values of all the different
    pairs of filters to get an idea of how correlated they are.
...
def compute_style_cost(model, STYLE_LAYERS):
    ...
    for layer_name, lambd in STYLE_LAYERS:
        ...
        # set a_S to be the hidden layer activation from selected layer
        a_S = sess.run(out)
...
def model_nst(sess, input_image, train_step, num_iterations=200, output_dir="./"):
...
if __name__ == "__main__":
    ...
    content_image = imread(sys.argv[1])
    cimage_origshape = content_image.shape
    ...
    STYLE_LAYERS = [("conv1_1", 0.2),  # (layer_name, lambda)
                    ...
                    ("conv4_2", 0.2),
                    ("conv5_1", 0.2)]
    # save these hyperparameters within the outputs' folder for reproducibility
    ...
    # run the model
    model_nst(sess, initialized_image, train_step, 200, output_folder)
    ...
    resized = pil_generated_image.resize((cimage_origshape[1],
                                          cimage_origshape[0]))
    resized = np.array(resized)
    imwrite(output_folder + "/generated_image_resized.jpg", resized,
            dpi=(600,600), quality=100)
