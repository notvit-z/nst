...
def compute_style_cost(model, STYLE_LAYERS):
    ...
    for layer_name, lambd in STYLE_LAYERS:
        ...
        # set a_S to be the hidden layer activation from the selected layer
        a_S = sess.run(out)
...
def model_nst(sess, input_image, num_iterations=200, output_dir="./outputs"):
    ...
    clear()
    print(f"frames interval: {frames_interval[0]}-{frames_interval[1]}\n")
    print(f"running frame {count+1-frames_interval[0]} / \
          {frames_interval[1]-frames_interval[0]}.\n")
    ...
    save_image(output_dir + "/generated_imageV.jpg", generated_image)
...
if __name__ == "__main__":
    ...
    frames_interval = [int(sys.argv[3]), int(sys.argv[4])+1]
    ...
    while(video.isOpened()):
        ...
        if count not in range(frames_interval[0], frames_interval[1]):
            continue
        ...
        # run the model
        model_nst(sess, initialized_image, 300, output_folder)
        ...
        imwrite(output_folder + "/frame_" + \
                str(count).zfill(len(str(nframes))) + ".jpg",
                resized, dpi=(600,600), quality=100)
        sess.close()
    video.release()
